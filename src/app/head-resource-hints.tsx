...
export function HeadResourceHints({ settings }: HeadResourceHintsProps) {
  ...
  // Get the main image and placeholder for preloading
  const mainImage = settings.hero.images.find(img => img.size === "lg" || img.size === "md")?.paths.webp;
  ...
  return (
    <>
      ...
      {domains.map((domain, i) => (
        domain && <link key={`preconnect-${i}`} rel="preconnect" href={`https://${domain}`} crossOrigin="anonymous" />
      ))}
      
      {/* Critical image preloads */}
      {placeholderImage && (
        <link rel="preload" as="image" href={placeholderImage} type="image/webp" fetchPriority="high" />
      )}
      ...
      <Script id="image-preloader" strategy="afterInteractive">{inlineScript}</Script>
    </>
  );
} 